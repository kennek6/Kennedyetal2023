...
```{r Fig 1a}
p.alpha.gwg <- data_0 %>%
  select("BMI_cat_0", "gain_cat", "Shannon", "Observed", "para", "sampleTime")%>%
  pivot_longer(cols = all_of(measures),
               names_to = "measure", 
               values_to = "value", 
               values_drop_na = TRUE)%>%
  filter(measure == "Shannon" & BMI_cat_0 == "optimal")%>%
  drop_na(gain_cat) %>%
  ggplot(aes(x=gain_cat, y=value, colour = gain_cat, shape = para))+
...
data_alpha <- data_0 %>%
  select("sampleTime", "BMI_cat_0", "gain_cat", "weight_gain", "Observed", "Shannon", "participantID",  "BMI_SM0", "para")%>%
  pivot_longer(cols = all_of(measures),
               names_to = "measure", 
               values_to = "value", 
               values_drop_na = TRUE)%>%
...
p.alpha.bmi <- data_alpha %>%
  ggplot(aes(x=BMI_SM0, y=value))+
  geom_point(alpha=0.7, show.legend = TRUE, aes(shape = para, colour = BMI_cat_0))+
  theme(
    ...
    axis.line = element_line(colour = "black"), 
    strip.background = element_blank(),
    legend.text=element_text(colour = c("grey5", "grey50"))
    ) +
  geom_smooth(data = data_alpha[which(data_alpha$para == 0),],
              method="lm", se = FALSE,
              show.legend = FALSE, colour = "grey5") + 
  geom_smooth(data = data_alpha[which(data_alpha$para == 1),],
              method="lm", se = FALSE,
              show.legend = FALSE, colour = "grey50") +
  ylab("Shannon Index")+
  xlab("Pre-pregnancy BMI")+
  facet_grid(~gain_cat,
             labeller = labeller(gain_cat = c("above"=" ")))+
  scale_shape_manual(values = c(19,8), 
                     labels = c("Primiparous", "Multiparous"),
                     name = "Parity")+
  scale_colour_manual(values = bmi_palette)+
