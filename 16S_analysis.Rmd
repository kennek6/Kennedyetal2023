...
summary(sample_sums(mat_rel_filt))
```
...
```{r}
t.meta <- as.tibble(sample_data(mat_filt))%>%
  ...
  labelled::set_variable_labels(
   ...
   # PI_baby_0 = "Birth Ponderal Index", 
    ...
    height_child_5 = "Infant height at 6 months (cm)",
    BMI_child_5 = "Infant BMI at 6 months",
    #PI_child_5 = "Infant Ponderal Index at 6 months"
    )%>%
  ungroup()

t.meta$origin <- recode_factor(t.meta$origin, `1` = "Germany", `2` = "Russia", `3` = "Poland", `4` = "Turkey", 
                                `6` = "Canada", `7` = "EU", `8` = "South-East Asia", `5` = "Others")

t.meta <- t.meta %>%
  labelled::set_variable_labels(origin = "Origin")
  

t.meta$BMI_cat_0 <- recode_factor(t.meta$BMI_cat_0, optimal = "<25", overweight = "25-30", obese = ">30")

t.meta <- t.meta %>%
  mutate(BMI_cat_0 = factor(BMI_cat_0, levels = c("<25", "25-30", ">30")))

table(t.meta$BMI_cat_0, t.meta$para)
```

```{r}
#create a list of variables to include in Table 1
listVars <- c("age", "para", "gravida", "BMI_SM0", "weight_gain", "gain_cat", "weight_retained",
              "dG", "modeSimple", "BMI_cat_0" 
              )
```

```{r normality tests}
#test variables for normal distribution
numVars <- listVars[-which(listVars %in% 
                             c("BMI_cat_0", "gravida", "modeSimple", "sex", "origin", "para", "gain_cat"))]
...
t0 <- t.meta %>%
  ...
  select(participant, para, all_of(listVars), -BMI_cat_0)%>%
  tbl_summary(
    by = para,
    missing = "no",  
    #type = types,
    statistic = statistics,
    value = list(modeSimple = "vaginal", sex = "female", gravida = "primigravida")
  )%>%
  bold_labels()%>%
  add_p(list(BMI_SM0~"kruskal.test",
        dG~"kruskal.test"))%>%
  add_q()%>%
...
t1 <- t.meta %>%
  select(participant, listVars, -para)%>%
  tbl_summary(
    by = BMI_cat_0,
    missing = "no",  
    #type = types,
...
table <- tbl_merge(
  ...
  tab_spanner = c("Parity", "Overall pBMI", "Primiparous pBMI", "Multiparous pBMI")
)%>%
  as_gt()
...
gt::gtsave(table, here::here("results", "Table1.html"))
```
...
m1 <- lmer(Shannon~ BMI_cat_0*gain_cat*para + (1|participantID), data_test)
m2 <- lmer(Shannon~ BMI_cat_0*para + (1|participantID), data_test)
m3 <- lmer(Shannon~ BMI_cat_0*gain_cat + (1|participantID), data_test)
...
```{r GetAlpha}
anova.shann <- lmer(Shannon~ BMI_SM0 + (1|participantID), data_0)
stats::anova(anova.shann)
r.squaredGLMM(anova.shann)
```
...
```{r}
data_alpha <- data_0 %>%
  select("sampleTime", "BMI_cat_0", "gain_cat", "weight_gain",
...
ggsave(here("figures", "figs1.tiff"), width = 8, height = 4, units = "in")
```
...
ps.adonis = with(df, adonis2(d ~ para*BMI_cat_0*gain_cat + sampleTime + participantID, 
                             data = df, permutations = 9999, strata = sampleTime))
ps.adonis
...
ps.adonis = with(df, adonis2(d ~ BMI_cat_0*gain_cat + participantID, 
                             data = df, permutations = 9999, strata = sampleTime))
ps.adonis
...
df = as(sample_data(dat_0), "data.frame")
d = phyloseq::distance(dat_0, "bray")
ps.adonis = with(df, adonis2(d ~ BMI_cat_0*gain_cat + participantID, data = df, permutations = 9999, strata = sampleTime))
ps.adonis
```
...
p.beta <- dist.sd.2 %>%
  ...
  geom_point(aes(alpha=0.7, colour=para_1), show.legend = TRUE)+
...
fig1 <- cowplot::plot_grid(p2, p.beta,
                           ncol = 1,
                           #align = 'h',
                           labels = c('','c'),
                           label_x = 0, label_y = 1,
...
gen_mean_tbf = gen_df %>%
  drop_na(BMI_cat_0) %>%
  filter(gain_cat == "above", sampleTime != "SM5")%>%
...
#ggsave(here("figures", "micro", "tbf_genmeans_BMIcat_mean.jpeg"), width=8, height=6.5)
#pdf(here("figures", "micro", "20220314", "tbf_gen_means_BMIcat.pdf"), width = 5.5, height = 7)
#gen_mean_tbf
#dev.off()
```
...
gt::gtsave(table, here::here("results", "Table2.html"))
```
...
```{r}
alpha <- data %>%
  select("sampleTime", "BMI_cat_0", "gain_cat", "sex", "Observed", "Shannon", "participantID", "birth_antibiotics",  "para")%>%
  pivot_longer(cols = all_of(measures),
               names_to = "measure", 
               values_to = "value", 
               values_drop_na = TRUE)%>%
...
```{r}
fig6a <- cowplot::plot_grid(gen_mean_tbf, gen_relabun,
                         labels = c("a", "b"),
                         rel_widths = c(1,2)
                         )
